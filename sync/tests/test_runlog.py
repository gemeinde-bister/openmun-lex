...
def test_start_run_log_creates_file_with_header(tmp_path: Path) -> None:
    path, handler = start_run_log(tmp_path, command="lex-sync sync")
    try:
    ...
    assert path.name.endswith("_vs.log")
    text = path.read_text(encoding="utf-8")
    assert "VS sync run started" in text
    assert "command: lex-sync sync" in text
